---
title: "Summary Statistics and Trends in the Homogeneous-Good E-Cigarette Price and Nicotine Series"
subtitle: "Companion note to *Dual Dynamics of U.S. Cigarette & E-Cigarette Demand* — the price/nicotine series, the open-refill and disposable-coverage caveats, tracked volume, and market concentration"
date: "2026"
geometry: margin=1in
fontsize: 11pt
---
...
In the demand side, agents in a state-month market choose among
...
form factor; the disposable-coverage comparison appears in §7.
...
surges via *size* (5–6 mL units). But see §7 — disposables are a minority of the
...
A note on brand composition first. In the *all-types* market (§6 concentration table),
...
"leadership" is a bottled-juice phenomenon that the closed-product (pod+disposable)
series does not see.
...
in context. Units are packages/cartridges as scanned (see §8, the unit-definition
...
2019. This is the volume counterpart to the mL-weighting point in §7 — the tracked
market is overwhelmingly a closed-product (pod, then pod+disposable) market by units.

## 6. Market concentration over time

The homogeneous-good series average over whichever brands are selling in a given
state-month, so it is worth documenting how concentrated that underlying brand set is.
The figure below plots the monthly Herfindahl–Hirschman index (HHI) of national brand
mL shares, mL-weighted to match $P_{\text{hom}}$/$N_{\text{hom}}$, on the standard
0–10,000 scale with the DOJ moderately-/highly-concentrated reference lines (1,500 and
2,500). Two series are shown: all brands (with the residual **UNKNOWN** bucket treated
as a single "brand," which understates true fragmentation) and identified brands only
(UNKNOWN dropped and shares renormalized).
...
- **2013 (BLU era).** One brand, BLU, holds ~56% of tracked mL; HHI sits at
  ~3,500–4,000 — highly concentrated.
- **2014–2017 (fragmentation).** As MISTIC, NJOY, LOGIC, and bottled-juice brands
  proliferate, no brand exceeds ~26% and HHI falls to ~1,400–1,600, near the
  moderately-concentrated floor.
- **2018–2020 (JUUL).** JUUL's rise re-concentrates the market; HHI jumps back above
  2,500 and the leader share peaks at ~47% (2018).
- **2021–2023 (VUSE/JUUL duopoly).** VUSE overtakes JUUL; the top two brands together
  hold ~88% of tracked mL, CR3 ≈ 87%, and HHI climbs to ~3,550 by 2023 — highly
  concentrated again.
...
One caveat looks ahead to §7: this HHI is computed on **tracked-retail mL**, where
modern disposables (Puff Bar, Elf Bar, Breeze, Lost Mary) are undercovered. Because those
...
2021–2023 HHI is an upper bound on concentration for the all-channels market, even as
it accurately describes the tracked-retail universe the demand model uses.

## 7. Disposables are under-represented in RMS
...
e-cigarette, with the disposable share a lower bound; the household panel purchases
come from the same tracked universe, so the descriptive series and the price index are
internally consistent.

## 8. The low RMS disposable share: unit-definition vs. channel coverage

The RMS disposable share in §7 sits well below the CDC/IRI benchmark. Two things could
...
al. 2023), whereas the §7 RMS series counts raw scanner packages, so a multi-pod pack
...
standardization is a comparability diagnostic only; it is **not** used in the demand
model, which keeps the raw series.
