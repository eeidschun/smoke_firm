## Corrected-nicotine T2 panel: mirrors build_t2_panel.R but (1) recomputes
...
## Writes to a NEW file (does NOT overwrite the canonical panel) + impact report
## + top-UPC-per-year audit (item 3).
...
t2_dir   <- file.path(firm_dir, "output", "prelim_analysis", "t2")
source(file.path(firm_dir, "t2_classify.R"))
source(file.path(firm_dir, "upc_overrides.R"))     # YIELD_FACTOR (0.68), overrides, EXCLUDE
...
impmap <- if (file.exists(impmap_path)) readRDS(impmap_path)$map %>% mutate(upc12 = pad12(upc)) %>%
    select(upc12, map_mL = mL_total_imputed, map_nicyield = nic_yield_tot_imputed, map_t2 = t2_type_override) else
    tibble(upc12=character(), map_mL=double(), map_nicyield=double(), map_t2=character())
...
input_files <- list.files(input_dir, pattern=paste0("^full_.*7467.*\\.RData$"), full.names=TRUE)
...
save(bt_month_t2, file=file.path(input_dir,"bt_month_t2_niccorr_from_full.RData"))
