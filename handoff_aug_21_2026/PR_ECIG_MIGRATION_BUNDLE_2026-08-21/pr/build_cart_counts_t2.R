...
source(file.path(firm_dir, "upc_overrides.R"))
...
impmap <- if (file.exists(impmap_path)) {
  readRDS(impmap_path)$map %>% mutate(upc12 = pad12(upc)) %>%
    select(upc12, map_mL = mL_total_imputed, map_nicyield = nic_yield_tot_imputed,
           map_t2 = t2_type_override)
} else tibble(upc12=character(), map_mL=double(), map_nicyield=double(), map_t2=character())
...
input_files <- list.files(input_dir, pattern="^full_.*7467.*\\.RData$", full.names=TRUE)
rows <- list()
agg <- function(f) {
  load(f)
  x <- rms_w_attr %>%
